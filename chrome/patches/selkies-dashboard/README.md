...
- Overlay files listed in `chrome/selkies-dashboard/OVERLAY` are never overwritten by sync (`jolee-settings.js`, `jolee-bridge.js`, `jolee-shims/`, `jolee-theme.css`, `jolee-sidebar-gutter.js`, …). Put hop-only hide-flags, brand CSS, dynamic sidebar gutter, and postMessage glue there.
